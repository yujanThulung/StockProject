...
const StockChart = () => {
  ...
  const setTicker = usePricePredictionStore(state => state.setTicker);
  const fetchAllData = usePricePredictionStore(state => state.fetchAllData);
  const historicalData = usePricePredictionStore(state => state.historicalData);

  console.log("This is historical data in stock chart: from front ", historicalData);
  ...
  // Initialize with default ticker only if not already set
  useEffect(() => {
  fetchAllData(ticker);
}, [ticker]);
  ...
  const transformedData = useMemo(() => {
    ...
    return historicalData.historical_data.map((item, index) => {
      ...
      const prevClose = index > 0 
        ? historicalData.historical_data[index - 1].close 
        : item.close;
      const change = prevClose !== 0 
        ? ((item.close - prevClose) / prevClose) * 100 
        : 0;
  ...
  // Update chart data only when transformedData changes
  useEffect(() => {
